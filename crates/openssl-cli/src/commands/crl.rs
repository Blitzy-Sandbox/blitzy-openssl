...
//!
//! This module is currently a *thin dispatch stub*: the full CRL
//! parsing / display / signing pipeline (RFC 5280 §5) is tracked for
//! production port from `apps/crl.c`.  The stub exists so that the
...
impl CrlArgs {
    ...
    /// Replaces `apps/crl.c:crl_main()` (full CRL inspection / generation
