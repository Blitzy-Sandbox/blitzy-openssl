...
//!
//! This module is currently a *thin dispatch stub*: the full RFC 5280
//! certificate-chain verification path (including OCSP stapling, CRL
//! checks, name constraints, policy mapping) is tracked for production
//! port from `apps/verify.c`.  The stub exists so that the
...
impl VerifyArgs {
    ...
    /// Replaces `apps/verify.c:verify_main()` (full chain verification
