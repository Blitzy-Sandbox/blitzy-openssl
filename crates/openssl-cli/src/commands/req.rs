...
//!
//! This module is currently a *thin dispatch stub*: the full CSR
//! generation, signing, and parsing pipeline (RFC 2986 / PKCS#10)
//! is tracked for production port from `apps/req.c`.  The stub
//! exists so that the `clap`-derived command dispatcher can wire the
//! subcommand into [`crate::commands::CliCommand::execute`]
//! (Rule R10 — Wiring Before Done) and so that integration tests can
//! verify the dispatch path end-to-end without depending on the full
//! handler.
//!
...
impl ReqArgs {
    ...
    /// Replaces `apps/req.c:req_main()` (full CSR pipeline) with a
    /// dispatch-only stub pending the production port.
