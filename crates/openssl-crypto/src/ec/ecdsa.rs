...
fn hmac_sha256_simple(key: &[u8], data_parts: &[&[u8]]) -> Vec<u8> {
    ...
    let mut inner_data = Vec::with_capacity(block_size + data_parts.iter().map(|d| d.len()).sum::<usize>());
...
fn sha256_simple(data: &[u8]) -> Vec<u8> {
    ...
    for chunk in padded.chunks_exact(64) {
        ...
        for idx in 0..64 {
            let big_sig1 = wv[4].rotate_right(6)
                ^ wv[4].rotate_right(11)
                ^ wv[4].rotate_right(25);
            ...
            let big_sig0 = wv[0].rotate_right(2)
                ^ wv[0].rotate_right(13)
                ^ wv[0].rotate_right(22);
