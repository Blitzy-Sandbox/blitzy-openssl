...
const FIPS_APPROVED_CIPHERS: &[&str] =
    &["AES-128-CBC", "AES-192-CBC", "AES-256-CBC", "DES-EDE3-CBC"];
...
impl CmacContext {
    ...
    fn apply_params(&mut self, params: &ParamSet) -> ProviderResult<()> {
        ...
        if params.contains(PARAM_CIPHER) {
            let cipher: String = params.get_typed(PARAM_CIPHER).map_err(|e| {
...
impl MacContext for CmacContext {
    ...
    fn init(&mut self, key: &[u8], params: Option<&ParamSet>) -> ProviderResult<()> {
        ...
        let cipher_name = self.cipher_name.as_ref().ok_or_else(|| {
            ProviderError::Init("CMAC requires a CBC cipher to be set before init".to_string())
        })?;
        ...
        let key_data = self
            .key
            .as_ref()
            .ok_or_else(|| ProviderError::Init("CMAC requires a non-empty key".to_string()))?;
